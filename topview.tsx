...
// useState is a React Hook used to store and manage changing data.

import { Search, Bell, User, LogOut, ChevronDown } from "lucide-react";
// Importing icons from lucide-react.

import { Currency, CURRENCIES } from "../utils";
// Currency → type (like 'USD' | 'EUR' | etc)
// CURRENCIES → array of available currency options


// This defines what props this component expects from parent
...
export function TopBar({ currentCurrency, onCurrencyChange, onLogout }: TopBarProps) {
    ...
    // State to control whether profile dropdown is open or closed
    const [isProfileOpen, setIsProfileOpen] = useState(false);

    // State to control whether profile settings section is expanded
        ...
        <header className="flex h-16 items-center justify-between border-b border-white/5 bg-slate-900/50 px-8 backdrop-blur-xl z-20">
            ...
            <div className="flex items-center gap-6">
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        {/* Looping through CURRENCIES array */}
                        {CURRENCIES.map((c) => (
                    ...
                    {isProfileOpen && (
                        <div className="absolute right-0 top-full mt-2 w-64 rounded-xl border border-white/10 bg-slate-900 p-2 shadow-xl backdrop-blur-xl">
                            ...
                            <div className="mt-2 pt-2 border-t border-white/5">
                                <button
                                    onClick={onLogout}
                                    // Calls logout function from parent
